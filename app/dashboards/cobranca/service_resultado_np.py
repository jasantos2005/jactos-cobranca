...
def get_kpis_resultado(data_ini="2026-01-01", data_fim=None):
    ...
    r = query_one(f"""
        SELECT
            ...
            SUM(CASE WHEN cc.status='A' AND pag.id_cliente IS NULL AND os34.id_cliente IS NOT NULL THEN 1 ELSE 0 END) AS em_retirada,
            SUM(CASE WHEN cc.status='I' AND pag.id_cliente IS NULL AND os34f.id_cliente IS NOT NULL THEN 1 ELSE 0 END) AS retirado_cancelado,
            SUM(CASE WHEN cc.status='I' AND pag.id_cliente IS NULL AND os34f.id_cliente IS NULL THEN 1 ELSE 0 END) AS cancelado,
            SUM(CASE WHEN cc.status='A' AND pag.id_cliente IS NULL AND os34.id_cliente IS NULL THEN 1 ELSE 0 END) AS pendente
        ...
        LEFT JOIN (SELECT DISTINCT id_cliente FROM ixcprovedor.su_oss_chamado WHERE id_assunto=34 AND status NOT IN ('F')) os34 ON os34.id_cliente=cc.id_cliente
        LEFT JOIN (SELECT DISTINCT id_cliente FROM ixcprovedor.su_oss_chamado WHERE id_assunto=34 AND status='F') os34f ON os34f.id_cliente=cc.id_cliente
...
def get_resultado_nunca_pagaram(data_ini="2026-01-01", data_fim=None, pagina=1, por_pagina=30):
    ...
    rows = query(f"""
        SELECT cc.id AS contrato_id, cc.id_cliente, c.razao,
               ...
               (SELECT o.id FROM ixcprovedor.su_oss_chamado o WHERE o.id_cliente=cc.id_cliente AND o.id_assunto=34 ORDER BY o.data_abertura DESC LIMIT 1) AS os34_id,
               (SELECT o.status FROM ixcprovedor.su_oss_chamado o WHERE o.id_cliente=cc.id_cliente AND o.id_assunto=34 ORDER BY o.data_abertura DESC LIMIT 1) AS os34_status,
