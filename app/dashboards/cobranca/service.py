...
def count_fila(filtros: FiltrosGlobais):
    ...
    if filtros.busca:
        ...
        b = f"%{filtros.busca}%"
        params += [b, b, b, b]
    filtro_filial = _filtro_filial(filtros.filial_id)
    sql = f"""
        ...
        WHERE c.ativo = 'S' AND cc.status = 'A'
          AND DATEDIFF(CURDATE(), f.data_vencimento) {{faixa}}
          AND f.id NOT IN ({{ph}}) {{busca_sql}} {{filtro_filial}}
    """.format(faixa=faixa, ph=ph, busca_sql=busca_sql, filtro_filial=filtro_filial)
...
def count_andamento(usuario_id: int = None, data_inicio: str = None, data_fim: str = None, filtros=None):
    wheres = ["i.pago=0", "(i.resolvido IS NULL OR i.resolvido=0)", "(i.segunda_cobranca IS NULL OR i.segunda_cobranca=0)"]
    if usuario_id:
        wheres.append(f"i.usuario_id={usuario_id}")
    if data_inicio:
    ...
    if data_fim:
        wheres.append(f"date(i.criado_em) <= '{data_fim}'")
    where_str = " AND ".join(wheres)
    rows = local_query(f"SELECT i.fn_areceber_id FROM cob_interacoes i WHERE {where_str}", ())
    if not rows:
        return 0
    fn_ids = tuple(r["fn_areceber_id"] for r in rows if r["fn_areceber_id"])
    if not fn_ids:
        return 0
    filial_id = filtros.filial_id if filtros else 0
    filial_and = f"AND cc.id_filial={filial_id}" if filial_id else ""
    ph = ",".join(["%s"]*len(fn_ids))
    r = query_one(f"""
        SELECT COUNT(*) AS total FROM ixcprovedor.fn_areceber f
        INNER JOIN ixcprovedor.cliente_contrato cc ON cc.id=f.id_contrato
        WHERE f.id IN ({ph}) {filial_and}
    """, fn_ids)
    return r["total"] if r else 0
