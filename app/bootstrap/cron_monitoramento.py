...
Regras:
- OS 190 aberta + cliente pagou → fecha OS 190 + fecha OS 34 (qualquer status)
- OS 190 aberta + cliente deve < 60d → aguarda
- OS 190 aberta + cliente deve >= 60d → fecha OS 190 + abre OS 34
- OS 34 (qualquer status) + cliente pagou → fecha OS 34
"""
import sys, os
...
def abrir_os_retirada_ixc(
    id_cliente: int,
    id_contrato: int,
    mensagem: str
) -> int:
    """Abre OS 34 diretamente no banco MySQL.
    ...
    try:
        ...
        if existente:
            log(
                f"  OS 34 já existe para cliente "
                f"{id_cliente}: #{existente['id']}"
            )
            return existente["id"]
        ...
        return nova["id"] if nova else 0

    except Exception as e:
        log(
            f"  [ERRO abrir_os_34] "
            f"cliente={id_cliente} contrato={id_contrato} erro={e}"
        )
        return 0

def registrar_evento(tipo, id_cliente, id_os_190, id_os_34, razao, valor, dias_atraso, obs):
    agora = now_br().strftime('%Y-%m-%d %H:%M:%S')
    local_execute("""
        INSERT INTO cob_monitoramento_os
        (executado_em, tipo, id_cliente, id_os_190, id_os_34, razao, valor, dias_atraso, obs)
        VALUES (?,?,?,?,?,?,?,?,?)
    """, (agora, tipo, id_cliente, id_os_190, id_os_34, razao, valor, dias_atraso, obs))

def retirada_acelerada_nunca_pagou():
    """
    Clientes ativados há 30+ dias que NUNCA pagaram nenhuma parcela.
    Abre OS 34 independente de ter OS 190.
    """
    from app.core.db import query, query_one, execute
    ...
    for c in candidatos:
        try:
            fat = query_one("""
                SELECT id, documento, id_contrato
            ...
            try:
                ...
                log(
                    f"  🚫 OS 34 bloqueada — {c['razao']}: "
                    f"não foi possível resolver contrato/filial: {e}"
                )
                continue

            msg = (
                f"RETIRADA ACELERADA — {c['razao']} ativado há "
                f"{c['dias_ativado']}d sem nenhum pagamento. "
                f"Total: R$ {float(c['total_aberto']):.2f}{fat_info}"
            )

            os34_novo = abrir_os_retirada_ixc(
                c["id_cliente"],
                id_contrato,
                msg
            )
            ...
            if os34_novo:
                log(
                    f"  ✅ OS 34 #{os34_novo} aberta — {c['razao']} "
                    f"({c['dias_ativado']}d sem pagar) — filial {id_filial}"
                )
                abertas += 1
        except Exception as e:
...
def main():
    ...
    # Uma única query traz: cliente, OS 190, OS 34 e dias de atraso
    clientes = query("""
        SELECT
            c.id AS id_cliente,
            c.razao,
            MAX(CASE WHEN o.id_assunto=190 AND o.status='A' THEN o.id END) AS os190_id,
            MAX(CASE WHEN o.id_assunto=34  AND o.status<>'F' THEN o.id END) AS os34_id,
    ...
    """, ())
    # Inclui também clientes com só OS 34 aberta que podem ter pago
    clientes_os34_only = query("""
        SELECT
            c.id AS id_cliente,
            c.razao,
            NULL AS os190_id,
    ...
    clientes = list(clientes) + [c for c in clientes_os34_only if c["id_cliente"] not in ids_ja]
    ...
    for cli in clientes:
        ...
        if not tem_divida:
            ...
            ok190 = ok34 = True
            if os190:
                ok190 = fechar_os_ixc(os190)
                if ok190: log(f"     OS 190 #{os190} fechada")
            if os34:
                ok34 = fechar_os_ixc(os34)
                if ok34: log(f"     OS 34 #{os34} fechada")
            if ok190 and ok34:
                registrar_evento('pagou', id_cli, os190, os34, razao, 0, 0, "Cliente quitou")
                pagaram += 1
            else:
                erros += 1
            continue
        ...
        if os190:
            if atraso >= 60:
                # +60 dias → fecha OS 190 e abre/mantém OS 34
                log(f"  🔴 RETIRADA: {razao} (#{id_cli}) — {atraso}d")
                ok = fechar_os_ixc(os190)
                if ok:
                    # Verifica se já tem OS 34 aberta OU finalizada nos últimos 30 dias
                    ...
                    if not os34_recente:
                        ...
                        fat = qo("""
                            SELECT id, documento, valor_aberto, id_contrato
                            FROM ixcprovedor.fn_areceber
                        ...
                        if not fat or not fat.get("id_contrato"):
                            log(
                                f"     🚫 BLOQUEADO: fatura sem contrato "
                                f"para cliente {id_cli}"
                            )
                            erros += 1
                            continue
                        ...
                        registrar_evento('retirada', id_cli, os190, os34_novo or None, razao, total, atraso, f"{atraso}d")
                        retiradas += 1
                    else:
                        log(f"     OS 34 #{os34_recente['id']} já existe — mantida")
                else:
                    erros += 1
            else:
